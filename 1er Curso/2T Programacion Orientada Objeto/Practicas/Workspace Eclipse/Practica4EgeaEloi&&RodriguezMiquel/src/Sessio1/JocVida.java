...
public class JocVida extends JFrame implements ActionListener {
	...
	private int nfila, ncol, ngen, nvius, cont=1, contVius; // valors
	...
	public void initComponents() {
		...
		ec.setLayout(new GridLayout(20, 1)); // fer una taula amb una columna i 13 files per posar la info
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == this.fgen) {
			if (this.cont == this.ngen || ferGeneracio() == true || contVius == 0) {
				fgen.setEnabled(false);
				cont++;
			} else {
				ferGeneracio();
	...
	public boolean ferGeneracio() {
		...
		for (int i = 0; i < this.Panell.length; i++) {
			for (int j = 0; j < this.Panell[i].length; j++) {
				if (Panell[i][j].getEstat() == false && quantesVeines(i, j) == 3) {
					contVius++;
					this.Panell[i][j].setViu();

				} else if ((Panell[i][j].getEstat() == true && quantesVeines(i, j) >= 4)
						|| (Panell[i][j].getEstat() == true && quantesVeines(i, j) <= 1)) {
					contVius--;
		...
		for (int i = 0; i < this.nfila; i++) {
			for (int j = 0; j < this.ncol; j++) {
				while (diferent != true) {
					if (this.Panell[i][j].getEstat() == aux[i][j].getEstat()) {
						diferent = false;
					} else {
						diferent = true;
	...
	public void copiar() { // copiar panell i fer tractament amb aux
		for (int i = 0; i < this.nfila; i++) {
			for (int j = 0; j < this.ncol; j++) {
				...
				// es lo mismo
				/*
				 * if (Panell[i][j].getEstat() == true) { aux[i][j].setViu(); } else {
				 * aux[i][j].setMort(); }
				 */
			}
		}
